...
for top_path in TOP_PATHS:
    m[top_path] = [
        ...
        for i in Path(f"genscan{os.sep}{top_path}").glob("*py")
        if not str(i).endswith("__init__.py")
    ]

ROOT_MODULES = list(Path("genscan").glob("*py"))


@pytest.mark.parametrize("category", TOP_PATHS)
def test_top_docs(category: str):
    file_path = os.path.join(DOC_SOURCE, f"genscan.{category}.rst")
...
def test_docs_probes(classname: str):
    file_path = os.path.join(DOC_SOURCE, f"genscan.probes.{classname}.rst")
    ...
    target_doc = f"genscan.probes.{classname}\n"
    assert (
...
def test_docs_detectors(classname: str):
    file_path = os.path.join(DOC_SOURCE, f"genscan.detectors.{classname}.rst")
    assert os.path.isfile(
        file_path
    ...
    target_doc = f"genscan.detectors.{classname}\n"
    assert (
        open(category_file, "r", encoding="utf-8").read().find(target_doc) != -1
...
def test_docs_harnesses(classname: str):
    file_path = os.path.join(DOC_SOURCE, f"genscan.harnesses.{classname}.rst")
    assert os.path.isfile(
        file_path
    ...
    target_doc = f"genscan.harnesses.{classname}\n"
    assert (
        open(category_file, "r", encoding="utf-8").read().find(target_doc) != -1
...
def test_docs_evaluators(classname: str):
    file_path = os.path.join(DOC_SOURCE, f"genscan.evaluators.{classname}.rst")
    assert os.path.isfile(
        file_path
    ...
    target_doc = f"genscan.evaluators.{classname}\n"
    assert (
        open(category_file, "r", encoding="utf-8").read().find(target_doc) != -1
...
def test_docs_generators(classname: str):
    file_path = os.path.join(DOC_SOURCE, f"genscan.generators.{classname}.rst")
    assert os.path.isfile(
        file_path
    ...
    target_doc = f"genscan.generators.{classname}\n"
    assert (
        open(category_file, "r", encoding="utf-8").read().find(target_doc) != -1
...
def test_docs_buffs(classname: str):
    file_path = os.path.join(DOC_SOURCE, f"genscan.buffs.{classname}.rst")
    assert os.path.isfile(
        file_path
    ...
    target_doc = f"genscan.buffs.{classname}\n"
    assert (
        open(category_file, "r", encoding="utf-8").read().find(target_doc) != -1
...
def test_core_config_options_explained():
    ...
    core_config_file_name = (
        genscan._config.transient.package_dir / "resources" / "genscan.core.yaml"
    )
    l1_nodes_to_check = []
    l2_nodes_to_check = []
