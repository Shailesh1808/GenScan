...
in `genscan`. `genscan` detectors must inherit from a class in here."""
...
class Detector(Configurable):
    ...
    def detect(self, attempt: genscan.attempt.Attempt) -> Iterable[float]:
...
class HFDetector(Detector, HFCompatible):
    ...
    def detect(self, attempt: genscan.attempt.Attempt) -> List[float]:
...
class StringDetector(Detector):
    ...
    def detect(
        self, attempt: genscan.attempt.Attempt, case_sensitive=False
    ) -> Iterable[float]:
        detector_results = []
