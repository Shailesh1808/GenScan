...
and also the model being evaluated; if genscan's lists are older than 
those in the model, there may be false positives (ie hits where there's 
no vulnerability); if the model data is older than genscan, there may be 
false negatives (genscan might miss hallucinated packages for which there 
has since been written something).

The default is to use a Pypi package list from 9th December 2021. You might
also try the HF Hub dataset `genscan-llm/pypi-20211209`. Pypi archives
...
class PythonPypi(PackageHallucinationDetector):
    ...
    DEFAULT_PARAMS = PackageHallucinationDetector.DEFAULT_PARAMS | {
        "dataset_name": "genscan-llm/pypi-20230724",
...
class RubyGems(PackageHallucinationDetector):
    ...
    DEFAULT_PARAMS = PackageHallucinationDetector.DEFAULT_PARAMS | {
        "dataset_name": "genscan-llm/rubygems-20230301",
        "language_name": "ruby",
    }
...
class JavaScriptNpm(PackageHallucinationDetector):
    ...
    DEFAULT_PARAMS = PackageHallucinationDetector.DEFAULT_PARAMS | {
        "dataset_name": "genscan-llm/npm-20240828",
...
class RustCrates(PackageHallucinationDetector):
    ...
    DEFAULT_PARAMS = PackageHallucinationDetector.DEFAULT_PARAMS | {
        "dataset_name": "genscan-llm/crates-20240903",
        "language_name": "rust",
    }
