...
""" Definitions of commands and actions that can be run in the genscan toolkit"""
...
def start_logging():
    ...
    log_filename = _config.transient.data_dir / "genscan.log"
    ...
    # genscanlogger = logging.FileHandler("genscan.log", encoding="utf-8")
    # genscanformatter = logging.Formatter("%(asctime)s  %(levelname)s  %(message)s")
    # genscanlogger.setFormatter(genscanformatter)
    # genscanlogger.setLevel(logging.DEBUG)

    # rootlogger = logging.getLogger()
    # for h in rootlogger.handlers[:]:
    #    rootlogger.removeHandler(h)
    # rootlogger.addHandler(genscanlogger)
...
def start_run():
    ...
    filename = f"genscan.{_config.transient.run_id}.report.jsonl"
    ...
    _config.transient.reportfile.write(
        json.dumps(
            {
                "entry_type": "init",
                "genscan_version": _config.version,
...
def end_run():
    ...
    msg = f"genscan run complete in {timetaken:.2f}s"
    print(f"✔️  {msg}")
    logging.info(msg)
...
def print_plugins(prefix: str, color):
    ...
    from genscan._plugins import enumerate_plugins
...
def plugin_info(plugin_name):
    from genscan._plugins import plugin_info
...
def probewise_run(generator, probe_names, evaluator, buffs):
    import genscan.harnesses.probewise

    probewise_h = genscan.harnesses.probewise.ProbewiseHarness()
    probewise_h.run(generator, probe_names, evaluator, buffs)


def pxd_run(generator, probe_names, detector_names, evaluator, buffs):
    import genscan.harnesses.pxd

    pxd_h = genscan.harnesses.pxd.PxD()
    pxd_h.run(
        generator,
        probe_names,
        detector_names,
...
def write_report_digest(report_filename, digest_filename):
    from genscan.analyze import report_digest
