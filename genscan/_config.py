...
from genscan import __version__ as version
...
@dataclass
class TransientConfig(genscanSubConfig):
...
system = genscanSubConfig()
run = genscanSubConfig()
plugins = genscanSubConfig()
reporting = genscanSubConfig()
...
# of passing the UA string to _genscan_user_agent() needs to exist, outside of
# _config.run.user_agent
REQUESTS_AGENT = ""


def _genscan_user_agent(dummy=None):
...
def set_http_lib_agents(agent_strings: dict):
    ...
    if "requests" in agent_strings:
        ...
        utils.default_user_agent = _genscan_user_agent
...
def load_base_config() -> None:
    global loaded
    settings_files = [str(transient.package_dir / "resources" / "genscan.core.yaml")]
...
def load_config(
    site_config_filename="genscan.site.yaml", run_config_filename=None
) -> None:
