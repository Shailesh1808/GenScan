...
PAYLOAD_SCHEMA = {
    "type": "object",
    "properties": {
        "genscan_payload_name": {"type": "string"},
...
class PayloadGroup:
    """Represents a configured group of payloads for use with genscan
    ...
    def _load(self):
        ...
        try:
            ...
            raise genscan.exception.PayloadFailure(msg) from fnfe

        except json.JSONDecodeError as jde:
            msg = "payload: JSON error:" + str(jde)
            logging.error(msg, exc_info=jde)
            raise genscan.exception.PayloadFailure("Payload JSON error") from jde
        ...
        if validation_result is not True:
            ...
            raise genscan.exception.PayloadFailure(
        ...
        if "detector_config" in loaded_payload:
            try:
                ...
                raise genscan.exception.PayloadFailure(msg) from te

        if (
            genscan._config.transient.reportfile is not None
            and not genscan._config.transient.reportfile.closed
        ):
            payload_stat = pathlib.Path(self.path).stat()
            genscan._config.transient.reportfile.write(
                json.dumps(
                    {
...
class Director:
        ...
        for payload_path in entries:
            with open(str(payload_path), "r", encoding="utf-8") as payload_path_file:
                try:
                    ...
                    # raise genscan.exception.PayloadFailure(msg) from exc
    ...
    def load(self, name) -> PayloadGroup:
        ...
        try:
            ...
            raise genscan.exception.PayloadFailure(msg) from ke

        except genscan.exception.genscanException as ge:
            msg = f"payload: Requested payload {name} not found at expected path {path}"
            logging.error(msg, exc_info=ge)
            raise genscan.exception.PayloadFailure(msg) from ge
