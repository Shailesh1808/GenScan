...
"""Functions for working with genscan plugins (enumeration, loading, etc)"""
...
from genscan.exception import genscanException, ConfigFailure
...
class PluginCache:
    ...
    def _enumerate_plugin_klasses(self, category: str) -> List[Callable]:
        ...
        base_mod = importlib.import_module(f"genscan.{category}.base")
        ...
        for module_filename in sorted(
            ...
            mod = importlib.import_module(f"genscan.{category}.{module_name}")
                ...
                match len(parts):
                    case 3:
                        try:
                            module = ".".join(parts[:-1])
                            klass = parts[-1]
                            imported_module = importlib.import_module(f"genscan.{module}")
...
def enumerate_plugins(
    ...
    genscan's plugins are organised into four packages - probes, detectors, generators
...
def load_plugin(path, break_on_fail=True, config_root=_config) -> object:
    ...
    try:
        ...
        match len(parts):
            case 2:
                ...
                generator_mod = importlib.import_module(
                    f"genscan.{category}.{module_name}"
                )
        ...
        if break_on_fail:
            ...
            return False
    module_path = f"genscan.{category}.{module_name}"
    try:
    ...
    try:
        ...
        if break_on_fail:
            raise genscanException(e) from e
        else:
            return False
