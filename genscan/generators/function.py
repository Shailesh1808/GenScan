...
```
import mymodule
import genscan.generators.function

g = genscan.generators.function.Single(name="mymodule#myfunction")
```

The target function is expected to take a string, and return a string.
Other arguments passed by genscan are forwarded to the target function.

Note that one can import the intended target module into scope and then
invoke a genscan run via genscan's cli module, using something like:
...
genscan.cli.main("--model_type function --model_name mymodule#function_name --probes encoding.InjectBase32".split())
```
...
class Single(Generator):
    ...
    doc_uri = "https://github.com/NVIDIA/genscan/issues/137"
