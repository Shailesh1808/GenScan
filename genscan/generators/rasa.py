...
class RasaRestGenerator(RestGenerator):
    ...
    * ``ratelimit_codes`` - Which endpoint HTTP response codes should be caught
    ...
    `example_rasa_service.json", we can invoke genscan as:

    genscan --model_type rest -G example_rasa_service.json
    ...
    DEFAULT_PARAMS = RestGenerator.DEFAULT_PARAMS | {
        ...
        "method": "post",
        "ratelimit_codes": [429],
        "req_template": json.dumps({"sender": "genscan", "message": "$INPUT"}),
        "request_timeout": 20,
        "response_json": True,
