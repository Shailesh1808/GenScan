...
if there are problems. Otherwise, please let us know if it's still not working!
 ...
 https://github.com/NVIDIA/genscan/issues
...
from genscan.exception import ModelNameMissingError, genscanException
from genscan.generators.base import Generator
from genscan.resources.api.huggingface import HFCompatible

models_to_deprefix = ["gpt2"]


class HFRateLimitException(genscanException):
    pass


class HFLoadingException(genscanException):
    pass


class HFInternalServerError(genscanException):
    pass
...
class OptimumPipeline(Pipeline, HFCompatible):
    ...
    def _load_client(self):
        ...
        if not torch.cuda.is_available():
            ...
            raise genscanException(message)
    ...
    def _call_model(
        ...
        if isinstance(response, dict):
            if "error" in response.keys():
                ...
                raise TypeError(
                    f"Unsure how to parse 🤗 API response dict: {response}, please open an issue at https://github.com/NVIDIA/genscan/issues including this message"
                )
        elif isinstance(response, list):
            return [g["generated_text"] for g in response]
        else:
            raise TypeError(
                f"Unsure how to parse 🤗 API response type: {response}, please open an issue at https://github.com/NVIDIA/genscan/issues including this message"
            )
