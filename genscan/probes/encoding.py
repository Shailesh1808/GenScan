...
Recommended genscan payloads to try:
...
def _load_payloads():
    ...
    if "encoding" in _config.plugins.probes:
        ...
        for payload_class in _config.plugins.probes["encoding"]["payloads"]:
            ...
            payloads += genscan.payloads.load(payload_to_add).payloads

            if payload_to_add in extra_payload_tags:
                extra_tags += extra_payload_tags[payload_to_add]

    if not payloads:
        payloads = genscan.payloads.load(payload_mapping["default"]).payloads
