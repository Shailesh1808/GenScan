...
from genscan.exception import PluginConfigurationError
import genscan.attempt
import genscan.resources.theme
...
class Probe(Configurable):
    ...
    def _attempt_prestore_hook(
        self, attempt: genscan.attempt.Attempt, seq: int
    ) -> genscan.attempt.Attempt:
    ...
    def _buff_hook(
        self, attempts: Iterable[genscan.attempt.Attempt]
    ...
    @staticmethod
    def _postprocess_buff(attempt: genscan.attempt.Attempt) -> genscan.attempt.Attempt:
    ...
    def _mint_attempt(self, prompt=None, seq=None) -> genscan.attempt.Attempt:
        """function for creating a new attempt given a prompt"""
        new_attempt = genscan.attempt.Attempt(
            probe_classname=(
                str(self.__class__.__module__).replace("genscan.probes.", "")
                + "."
                + self.__class__.__name__
            ),
            goal=self.goal,
            status=genscan.attempt.ATTEMPT_STARTED,
            seq=seq,
            prompt=prompt,
        )
    ...
    def _execute_all(self, attempts) -> Iterable[genscan.attempt.Attempt]:
        """handles sending a set of attempt to the generator"""
        attempts_completed: Iterable[genscan.attempt.Attempt] = []
        ...
        if (
            ...
            attempt_bar.set_description(self.probename.replace("genscan.", ""))
            ...
            attempt_iterator.set_description(self.probename.replace("genscan.", ""))
...
class TreeSearchProbe(Probe):
    ...
    def probe(self, generator):
        ...
        self.generator = generator
        detector = genscan._plugins.load_plugin(f"detectors.{self.primary_detector}")

        all_completed_attempts: Iterable[genscan.attempt.Attempt] = []
        ...
        tree_bar = tqdm.tqdm(
            ...
            leave=False,
            colour=f"#{genscan.resources.theme.PROBE_RGB}",
        )
        ...
        while len(nodes_to_explore):
            ...
            for attempt in attempts_completed:
                ...
                attempt.status = genscan.attempt.ATTEMPT_COMPLETE
