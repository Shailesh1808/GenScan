...
"""Flow for invoking genscan from the command line"""
...
def main(arguments=None) -> None:
    """Main entry point for genscan runs invoked from the CLI"""
    import datetime

    from genscan import __description__
    ...
    import genscan.command as command
    import logging
    import re
    ...
    parser = argparse.ArgumentParser(
        prog="python -m genscan",
        description="LLM safety & security scanning tool",
        epilog="See https://github.com/NVIDIA/genscan",
        allow_abbrev=False,
    )
    ...
    parser.add_argument(
        ...
        "-r",
        type=str,
        help="process genscan report into a list of AVID reports",
    )
    parser.add_argument(
        "--interactive",
        "-I",
    ...
    parser.add_argument(
        "--interactive.py",
        action="store_true",
        help="Launch genscan in interactive.py mode",
    )
    ...
    try:
        ...
        # process commands
        if args.interactive:
            from genscan.interactive import interactive_mode
        ...
        if args.version:
            ...
            #
            # How should this handle genscan.site.yaml? Only if --fix was provided and no other options offered?
            ...
            if has_config_file_or_json:
                ...
                # check if genscan.site.yaml needs to be fixed up?
            ...
            from genscan.report import Report

            report_location = args.report
            print(f"📜 Converting genscan reports {report_location}")
        ...
        # model is specified, we're doing something
            ...
            for probe in parsed_specs["probe"]:
                ...
                if (
                    ...
                    is not None  # genscan.core.yaml always provides run.generations
                ):
                    _config.plugins.probes[p_module][p_klass][
                        "generations"
                    ] = _config.run.generations

            evaluator = genscan.evaluators.ThresholdEvaluator(_config.run.eval_threshold)
            ...
            if "generate_autodan" in args and args.generate_autodan:
                from genscan.resources.autodan import autodan_generate
        ...
        logging.info(msg)
        print(msg)
    except (ValueError, genscanException) as e:
        logging.exception(e)
        print(e)
