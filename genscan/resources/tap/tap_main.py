...
import genscan.generators.openai
...
from genscan.generators.huggingface import Model

logger = getLogger(__name__)
SAVE_RESULTS = True

resources_tap_data_file = (
    genscan._config.transient.cache_dir / "data" / "tap" / "tap_jailbreaks.txt"
)
...
class AttackManager(EvaluationJudge):
    def __init__(
        self,
        goal: str,
        attack_generator: genscan.generators.Generator,
        target_generator: genscan.generators.Generator,
        evaluation_generator: genscan.generators.Generator,
        ...
        for _ in range(self.attack_max_attempts):
            ...
            for left in range(0, len(full_prompts_subset), self.max_parallel_streams):
                ...
                for full_prompt in full_prompts_subset[left:right]:
                    # We should fail more gracefully within runs for genscan.
                    try:
                        outputs_list.append(
...
def run_tap(
    ...
    target_generator : genscan generator object for target
    ...
    if not isinstance(evaluator_generator, genscan.generators.openai.OpenAICompatible):
