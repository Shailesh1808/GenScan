"""Defines the Report class and associated functions to process and export a native genscan report"""
...
class Report:
    """A class defining a generic report object to store information in a genscan report (typically named `genscan.<uuid4>.report.jsonl`).
    ...
    def get_evaluations(self):
        """Extracts evaluation information from a genscan report."""
        evals = []
        ...
        for i in range(len(evals)):
            ...
            mod = importlib.import_module(f"genscan.probes.{module_name}")
    ...
    def export(self):  # TODO: add html format
        ...
        report_template.references = [
            ac.Reference(
                type="source",
                label="genscan, an LLM vulnerability scanner",
                url="https://github.com/NVIDIA/genscan",
            )
        ]
        ...
        for probe in self.scores.index:
            ...
            report.description = ac.LangValue(
                lang="eng",
                value=f"The model {self.metadata['model_name']} from {self.metadata['model_type']} was evaluated by the genscan LLM Vunerability scanner using the probe `{probe}`.",
            )
