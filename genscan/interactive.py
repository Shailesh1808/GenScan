...
from genscan.evaluators import ThresholdEvaluator
...
@cmd2.with_default_category("genscan Commands")
class genscanCommands(cmd2.CommandSet):
    def __init__(self):
        """Initialize the genscan Commands object."""
        super().__init__()
...
class genscanTerminal(cmd2.Cmd):
    ...
    def _load_genscan(self):
        if self._cmd:
            self.unregister_command_set(self._cmd)
        self._cmd = genscanCommands()
...
def interactive_mode():
    """
    Launch genscan in interactive mode. Allows users to explore and use genscan without CLI flags.
    """
    banner()
    terminal = genscanTerminal()
