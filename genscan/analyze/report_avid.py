...
"""Prints a report given a genscan report in jsonl"""
...
parser = argparse.ArgumentParser(
    description="Conversion Tool from native genscan jsonl to AVID reports"
)
...
parser.add_argument(
    ...
    "-r",
    type=str,
    help="location of genscan report file",
)
...
for probe in probe_scores.index:
    ...
    report.description = LangValue(
        lang="eng",
        value=f"The model {meta['model_name']} from {meta['model_type']} was evaluated by the genscan LLM Vunerability scanner using the probe `{probe}`.",
    )
