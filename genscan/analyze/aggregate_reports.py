...
"""
 aggregate multiple genscan reports on the same generator 
...
p = argparse.ArgumentParser(
    description="aggregate multiple similar genscan reports into one jsonl"
)
p.add_argument("-o", help="output filename", required=True)
p.add_argument("infiles", nargs="+", help="genscan jsonl reports to be aggregated")
a = p.parse_args()
...
with open(a.o, "w", encoding="utf-8") as out_file:
    ...
    with open(in_filenames[0], "r", encoding="utf8") as lead_file:
        # extract model type, model name, genscan version
        ...
        assert init["genscan_version"] == version
