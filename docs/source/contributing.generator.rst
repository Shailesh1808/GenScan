...
Let's build a genscan generator. In genscan, generators provide abstraction between the main code base and a dialogue system. That system is often an LLM, but can be anything that accepts and returns text (or text plus some other modality).
...
All generators in genscan must descend from ``genscan.generators.base.Generator``. So, new generators start out like:
...
Class docstrings are mandatory in genscan, enforced by a test that's required to pass before merging. And they're just sensible practice anyway. So we add a brief docstring here too.

It's important that generators always inheirt from ``genscan.generators.base.Generator``. Not only does this often save a lot of work - using the logic in base's ``generate()`` is helpful - plugin listing also checks the parents of each class in each plugin when choosing to show a plugin as available. If there's a class in a module in ``genscan/generators/`` that doesn't inherit from Generator, then it won't be listed as available.


Setting the default generator
*****************************

Before we go ahead and code up the generator, we need to tell genscan which generator to use by default in this class. This helps save users from always having to specify a model name. We do this in a default class module variable:

.. code-block:: python

    DEFAULT_CLASS = "ReplicateGenerator"

When this generator module is selected using ``--model_type replicate``, with ``replicate`` being the name of the Python module we're working in, the generator loader (in ``genscan/generators/__init__.py``) will search for the ``DEFAULT_CLASS`` constant and use its value to determine which class to instantiate.

Configuration
*************

Like many genscan plugins, generators are configurable. This is achieved by the base class inheriting from ``Configurable``. So, the first code we see in a generator class looks like this:
...
In the class, we'll set the ``generator_family_name`` to ``Replicate``. This can be the same for every generator in a module, or can vary a bit, but it should be a descriptive name that reflects what this class is for - the generator family name is printed to genscan users at run time. Finally, because Replicate endpoints aren't guaranteed to support a single request for multiple generations at a time, we set ``supports_multiple_generations`` false. This is also the default value, so technically we don't have to do it, but it's OK to be explicit here.
...
(from genscan/generators/langchain_serve.py)
...
(genscan/generators/nemo.py)


Finally, if the key check passed, let's try to load up the Replicate API using the ``replicate`` module and the user-supplied key. We don't want to do speculative loading in genscan - everything should be imported as late as reasonable, to keep user experience fast.
...
The core part of getting a result out of LLMs represented using the Replicate API is to submit a text prompt, and capture a single response to that. Within genscan, functionality is handled by ``Generator``'s private ``_call_model()`` method - and so that's what we will overload in the ``ReplicateGenerator`` class.
...
The ``max_value`` param means to never wait more than 70 seconds. API modules like Replicate's often use the ``logging`` module to give more detailed info, which is stored in ``genscan.log``, if one wants to troubleshoot.
...
Now that the pieces for our generator are in place - a subclass of ``genscan.generators.base.Generator``, with some customisation in the constructor, and an overridden ``_call_model()`` method, plus a ``DEFAULT_CLASS`` given at module level - we can start to test.

A good first step is to fire up the Python interpreter and try to import the module. genscan supports a specific range of tested Python versions (listed in `pyproject.toml <https://github.com/NVIDIA/genscan/blob/main/pyproject.toml>`_, under the ``classifiers`` descriptor), so remember to use the right Python version for testing.
...
.. code-block:: bash
    ...
    >>> import genscan.generators.replicate
    >>> 
...
.. code-block:: bash
    ...
    >>> g = genscan.generators.replicate.ReplicateGenerator("meta/llama-2-70b-chat")
    Traceback (most recent call last):
    File "<stdin>", line 1, in <module>
    File "/home/lderczynski/dev/genscan/genscan/generators/replicate.py", line 44, in __init__
        super().__init__(name, generations=generations, config_root=config_root)
    File "/home/lderczynski/dev/genscan/genscan/generators/base.py", line 43, in __init__
        self._load_config(config_root)
    File "/home/lderczynski/dev/genscan/genscan/configurable.py", line 60, in _load_config
        self._validate_env_var()
    File "/home/lderczynski/dev/genscan/genscan/configurable.py", line 116, in _validate_env_var
        raise APIKeyMissingError(
    genscan.exception.APIKeyMissingError: 🛑 Put the Replicate API key in the REPLICATE_API_TOKEN environment variable (this was empty)
...
The next step is to try some integration tests - executing genscan from the command line, accessing this generator. There are some pointers in :doc:`contributing`. You might need to execute genscan by specifying it as a Python module, running the command from the genscan root code directory. Things to test are:

* Does the new generator appear in ``python -m genscan --list_generators``?
* Does the generator work with a test probe, via ``python -m genscan -m replicate -n meta/llama-2-70b-chat -p test.Blank``?
...
If you want to see the full, live code for the Replicate genscan generator, it's here: `genscan/generators/replicate.py <https://github.com/NVIDIA/genscan/blob/main/genscan/generators/replicate.py>`_ .

Done!
=====

Congratulations - you've written a genscan plugin!

If it's all tested and working, then it's time to send the code. You should first run ``black`` to format your code in the standard that the genscan repository expects (Python 3.10 style, 88 columns). Then, push your work to your github fork, and finally, send us a pull request - and we'll take it from there!


Advanced: Modalities
====================

This tutorial covered a tool that takes text as input and produces text as output. genscan supports multimodality - the kinds of format that a generator supports are covered in a modality dictionary, with two keys, in and out. The default is:

.. code-block::

    modality: dict = {"in": {"text"}, "out": {"text"}}

For an example of a multimodal model, check out LLaVa in `genscan.generators.huggingface <https://github.com/NVIDIA/genscan/blob/main/genscan/generators/huggingface.py>`_ . 