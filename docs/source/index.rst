Welcome to the genscan reference documentation
============================================

**genscan** is an LLM vulnerability scanner, `<https://genscan.ai>`_.
It uses a huge range of probes to examine and query a large language model, simulating
attacks, and uses a range of detectors on the model's outputs to see if the model was 
vulnerable to any of those attacks.

This is the code reference documentation, mostly useful for developers and people interested
in how genscan works. There is a separate `User Guide <https://docs.genscan.ai>`_ containing information
on running genscan and interpreting results. If you want to use the tool and get results,
and you don't care about its internals, then you want the user guide. Take a look there! `<https://docs.genscan.ai>`_

On the other hand, if you'd like a to get into the details or work out how 
to contribute code, you're in the right place - welcome!

You can also join our `Discord <https://discord.gg/uVch4puUCs>`_
and follow us on `Twitter <https://twitter.com/genscan_llm>`_!

Check out the :doc:`usage` section for further information, including :ref:`installation`.

.. note::

   This project is under active development. We love writing and fixing docs, so
   let us know if there's anything wrong, confusing, or missing here --
   mail `docs@genscan.ai <mailto:docs@genscan.ai>`_ or drop us a note on `Discord <https://discord.gg/uVch4puUCs>`_. 
   Thank you!

Contents
--------


Using genscan
^^^^^^^^^^^

.. toctree::
   :maxdepth: 1

   how
   usage
   FAQ <https://github.com/NVIDIA/genscan/blob/main/FAQ.md>

Advanced usage
^^^^^^^^^^^^^^

.. toctree::
   :maxdepth: 1

   configurable
   cliref

Code reference
^^^^^^^^^^^^^^

.. toctree::
   :maxdepth: 1

   contributing

   basic
   attempt
   cli
   command
   exception
   interactive
   payloads
   _config
   _plugins

Plugin structure
^^^^^^^^^^^^^^^^

.. toctree::
   :maxdepth: 1

   buffs
   detectors
   evaluators
   generators
   harnesses
   probes
   report
