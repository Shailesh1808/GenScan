...
Beyond the standard CLI options, genscan is highly configurable.
You can use YAML files to configure a genscan run, down to the level
...
``genscan`` can be configured in multiple ways:
...
genscan's config hierarchy
^^^^^^^^^^^^^^^^^^^^^^^^

Configuration values can come from multiple places. At genscan load, the
...
3. Values in a YAML site config, ``genscan.site.yaml``, placed in the config directory (``XDG_CONFIG_DIR``, which is ``~/.config/genscan/`` on Linux; see XDG spec for details)
4. Fixed values kept in the genscan core config - don't edit this. Package updates will overwrite it, and you might break your genscan install. It's in ``genscan/resources`` if you want to take a look.
...
* ``parallel_attempts`` - For parallelisable generators, how many attempts should be run in parallel? Raising this is a great way of speeding up genscan runs for API-based models
...
* ``show_z`` - Display Z-scores and visual indicators on CLI. It's good, but may be too much info until one has seen genscan run a couple of times
* ``enable_experimental`` - Enable experimental function CLI flags. Disabled by default. Experimental functions may disrupt your installation and provide unusual/unstable results. Can only be set by editing core config, so a git checkout of genscan is recommended for this.
...
* ``user_agent`` - What HTTP user agent string should genscan use? ``{version}`` can be used to signify where genscan version ID should go
...
""""""""""""""""""""""""""
* ``report_dir`` - Directory for reporting; defaults to ``$XDG_DATA/genscan/genscan_runs``
* ``report_prefix`` - Prefix for report files. Defaults to ``genscan.$RUN_UUID``
...
genscan comes bundled with some quick configs that can be loaded directly using ``--config``.
...
These are great places to look at to get an idea of how genscan YAML configs can look.
Quick configs are stored under ``genscan/configs/`` in the source code/install.
...
If we save this as ``latent1.yaml`` somewhere, then we can use it with ``genscan --config latent1.yaml``.
...
genscan's functions are through its plugins. Most parts of genscan are plugins,
...
You can see the parameters for any given plugin using genscan ``--plugin_info``.

.. code-block:: text

    $ python -m genscan --plugin_info probes.grandma.Win10
    genscan LLM vulnerability scanner v0.10.0 ( https://github.com/NVIDIA/genscan ) at 2024-09-11T11:54:07.215242
...
The preferred way to instantiate a plugin is using ``genscan._plugins.load_plugin()``.
This function takes two parameters:

* ``name``, the plugin's package, module, and class - e.g. ``generator.test.Lipsum``
* (optional) ``config_root``, either genscan._config or a dictionary of a config, beginning at a top-level plugin type.
...
.. code-block:: python
    ...
    >>> genscan._plugins.load_plugin("generators.openai.OpenAIGenerator", config_root=c)
    🦜 loading generator: OpenAI: gpt-4
    <genscan.generators.openai.OpenAIGenerator object at 0x71bc97693d70>
