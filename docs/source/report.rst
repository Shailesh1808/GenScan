...
``genscan``'s reports connect to things interested in consuming info on 
LLM vulnerabilities and failures, such as the AI Vulnerability Database.

``genscan`` provides a CLI option to further structure this file for downstream consumption. 
...
.. code-block:: console
   ...
   python3 -m genscan -r <path_to_file>

Examples
^^^^^^^^

As an example, let's load up a ``genscan`` report from scanning ``gpt-3.5-turbo-0613``.
...
.. code-block:: console
   ...
   python3 -m genscan -r gpt35-0906.report.jsonl
...
.. code-block:: console
   ...
   📜 Converting genscan reports gpt35-0906.report.jsonl
