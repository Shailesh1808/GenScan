...
with ``genscan``. Payloads are items intended for inserting in prompts, that
...
probes. The search priority path is:
    ...
    1. genscan._config.transient.data_dir / "resources" / "payloads",
    2. genscan._config.transient.package_dir / "resources" / "payloads",

That is, user payloads under XDG_DATA_DIR override those provided with genscan.
...
.. code-block:: JSON
    ...
    {
        "genscan_payload_name": -- a mandatory key, used to identify this as a genscan payload. holds a description of the payload.
